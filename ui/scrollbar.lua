...
function scrollbar.new(axis, onScroll)
  ...
  { axis = axis, onScroll = onScroll, scrollX = 0, scrollY = 0, scrollWidth = size, scrollHeight = size }, scrollbar)
  if axis == "x" then
    self.height = size
  else
    self.width = size
...
function scrollbar:draw()
  ...
  lg.setColor(c, c, c)
  lg.rectangle("fill", self.scrollX + padding, self.scrollY + padding, self.scrollWidth - padding * 2,
    self.scrollHeight - padding * 2, cornerRadius)
  lg.pop()
end
